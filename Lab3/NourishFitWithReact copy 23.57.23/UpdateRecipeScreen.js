...
const UpdateRecipeScreen = ({route, navigation}) => {
  const {id} = route.params; // Retrieve the ID from navigation route params
  const {recipes, updateRecipe} = useContext(RecipeContext);

  // Fetch the selected recipe by ID
  ...
  const handleUpdate = () => {
    updateRecipe(id, updatedRecipe); // Update the recipe using the updateRecipe function from context
    navigation.goBack(); // Navigate back to the previous screen after updating the recipe
  };
